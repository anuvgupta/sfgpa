...
var toolNames = ['HOME', 'GPA CALCULATOR', 'FINAL GRADES', 'WORD COUNTER', 'EASYBIB CITE'];
		...
		$('#lHamburger').click(function () {
			if(menuDown) {
				...
				setTimeout(function () {
					...
					$('.link').css({'transition':'fontSize 0.35s ease-in-out', '-moz-transition':'fontSize 0.35s ease-in-out', '-ms-transition':'fontSize 0.35s ease-in-out', '-webkit-transition':'fontSize 0.35s ease-in-out', '-o-transition':'fontSize 0.35s ease-in-out'});
...
function resize() {
	if (window.innerWidth < window.innerHeight) {
		...
		if (mobileView) _('lSpacer').innerHTML = "<center><span id = 'mobileTitle'>" + toolNames[curTool] + '</span></center>';
		setEvents();
	} else {
		mobileView = false;
		_('menu').style.height = '73px';
